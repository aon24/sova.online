...
window.sovaActions.signup = {
	cmd: {
		proceed: doc => {
			...
			if (!phone)
				return doc.msg.ok('Регистрация только для студентов "Нового Века"','Регистрация|Телефон не указан');
			
			fetch(`/api/getData?form=login&cmd=checkPhone&phone=${phone}`, {method: 'get', credentials: 'include'})
				.then( response => response.text() )
				.then( tx => {
					if (tx === 'not found')
						return doc.msg.ok('Регистрация только для студентов "Нового Века"','Регистрация|Телефон не найден');
					if (tx === 'exist')
