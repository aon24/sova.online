...
window.sovaActions.etc = {
    init2: doc => {
		...
		for (let it of ['eMovePl_etc', 'eSavePl_etc', 'eBtnClose_etc']) {
			let i = localStorage.getItem(it);
			doc.setField(it, i === '0' ?  '' : i);
		}
	},
	...
	recalc: {
		...
		EBTNCLOSE_ETC: (doc, val) => doc.mainDoc.eBtnClose_etc = val,
		SCALE_ETC: (doc, val) => {
	...
	cmd: {
		previewArm: (doc, url, ctrlKey, shift) => {
            let kv = doc.util.urlKeys(url);
            ctrlKey ?
                doc.util.xopen(`/api/new?form=${kv.newForm}&dbAlias=${kv.dbAlias}&unid=1&mode=read`)
                :
                doc.previewNew(kv, ctrlKey, shift);
		},
